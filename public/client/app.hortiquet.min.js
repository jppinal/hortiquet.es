...
b.post("/paypal-create",shoppingData).then(function(a){console.log(a);d.location.href=a.data.href},function(a){console.log(a)})}}]).directive("confirmStep",function(){return{restrict:"E",transclude:!0,scope:{},templateUrl:"/client/shopping/confirm/template.confirm.step.html",controller:"ConfirmStepController"}});angular.module("order").controller("OrderController",["$scope","ShoppingFactory",function(a,b){a.cart=b.cart;a.cart.clearItems()}]);angular.module("product").controller("ProductsController",["$scope","$location","$anchorScroll","Authentication","ShoppingFactory",function(a,b,c,d,f){a.authentication=d;a.cart=f.cart;d=$(".left-control");3>d.length&&d.addClass("hidden");d=$(".right-control");3>d.length&&d.addClass("hidden");a.addtoCart=function(d){a.cart.addItem(d);b.hash("top");c();$("#md-cart").addClass("animated tada").one("webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend",function(){$("#md-cart").removeClass("animated tada")});
$("#sm-cart").addClass("animated tada").one("webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend",function(){$("#sm-cart").removeClass("animated tada")})}}]);angular.module("home").controller("HomeController",["$scope","Authentication","ShoppingFactory",function(a,b,c){a.authentication=b;a.cart=c.cart;a.resizeHomeItems=function(){var a=$(window).height();$("#home-slider .item").css("height",a)}}]);angular.module("home").config(["$routeProvider",function(a){a.when("/",{templateUrl:"/client/home/view.home.client.html",controller:"HomeController"}).when("/legal-notice",{redirectTo:"/legal/notice"}).when("/privacy-policy",{redirectTo:"/legal/privacy"}).when("/privacy-cookies",{redirectTo:"/legal/cookies"})}]);angular.module("legal").controller("LegalsController",["$scope","Authentication",function(a,b){a.authentication=b;a.website="www.hortiquet.es";a.webAbrv="SITIO WEB";a.legalResponsible="Nombre Apellidos con DNI-123456789A";a.legalAbrv="UMVELTA"}]);angular.module("legal").config(["$routeProvider",function(a){a.when("/notice",{templateUrl:"/client/legal/views/view.notice.legal.html",controller:"LegalsController"}).when("/privacy-policy",{templateUrl:"/client/legal/views/view.policy.legal.html",controller:"LegalsController"}).when("/privacy-cookies",{templateUrl:"/client/legal/views/view.cookies.legal.html",controller:"LegalsController"})}]);var appName="hortiquet",app=angular.module(appName,"ngResource ngRoute angular-cookie-law common users shopping home product order legal".split(" "));app.config(["$locationProvider",function(a){a.hashPrefix("!")}]);"#_=_"===window.location.hash&&(window.location.hash="#!");angular.element(document).ready(function(){angular.bootstrap(document,[appName])});
